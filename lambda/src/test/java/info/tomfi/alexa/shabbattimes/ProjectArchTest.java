...
class ProjectArchTest {
  ...
  @ArchTest
  final ArchRule custom_exceptions_should_only_be_thrown_by_handlers_and_services =
  ...
  @ArchTest
  final ArchRule custom_exceptions_should_be_public_and_extend_the_ask_sdk_exception =
      classes().that().resideInAPackage("..shabbattimes.exceptions..")
          .should().bePublic().andShould().beAssignableTo(AskSdkException.class)
          .as("exception handling is mostly performed by the api");

  @ArchTest
  final ArchRule custom_handlers_interceptors_and_services_should_be_marked_as_di_components =
  ...
  @ArchTest
  final ArchRule custom_handlers_interceptors_and_services_should_not_be_manually_accessed =
      layeredArchitecture()
  ...
  @ArchTest
  final ArchRule exception_handlers_should_be_public_and_implment_exception_handler_interface =
      classes().that().resideInAPackage("..shabbattimes.handlers.exception..")
          .should().bePublic().andShould().implement(ExceptionHandler.class)
          .as("exception handlers should be accepted by the api");

  @ArchTest
  final ArchRule intent_handlers_should_be_public_and_implment_intent_request_handler_interface =
      classes().that().resideInAPackage("..shabbattimes.handlers.intent..")
          .should().bePublic().andShould().implement(IntentRequestHandler.class)
          .as("intent request handlers should be accepted by the api");

  @ArchTest
  final ArchRule launch_handlers_should_be_public_and_implment_launch_request_handler_interface =
      classes().that().resideInAPackage("..shabbattimes.handlers.launch..")
          .should().bePublic().andShould().implement(LaunchRequestHandler.class)
          .as("launch request handlers should be accepted by the api");

  @ArchTest
  final ArchRule sssion_handlers_should_be_public_and_implmnt_sssion_ended_request_handler_intrfce =
      classes().that().resideInAPackage("..shabbattimes.handlers.session..")
          .should().bePublic().andShould().implement(SessionEndedRequestHandler.class)
          .as("session ended request handlers should be accepted by the api");

  @ArchTest
  final ArchRule request_interceptors_should_be_public_and_implment_request_interceptor_interface =
      classes().that().resideInAPackage("..shabbattimes.interceptors.request..")
          .should().bePublic().andShould().implement(RequestInterceptor.class)
          .as("request interceptors should be accepted by the api");

  @ArchTest
  final ArchRule response_intercptors_should_be_public_and_implment_response_interceptor_interface =
      classes().that().resideInAPackage("..shabbattimes.interceptors.response..")
          .should().bePublic().andShould().implement(ResponseInterceptor.class)
          .as("response interceptors should be accepted by the api");

  @ArchTest
  final ArchRule services_should_be_public =
      classes().that().resideInAPackage("..shabbattimes.services..").should().bePublic();
}
